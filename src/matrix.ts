      ...
      return (() => {
        ...
        for (var str of Array.from(strings)) {
          ...
          if (/^(f|ht)tps?:\/\//i.test(str)) {
            ...
            result.push(
              that.client.sendNotice(envelope.room, str).catch((err) => {
              ...
              .then((content_uri) => {
                return that.client
                  ...
                  .catch((err) => {
                    if (err.name === "UnknownDeviceError") {
                      that.handleUnknownDevices(err);
                      return that.client.sendImageMessage(
                        envelope.room,
                        content_uri,
                        info,
                        url
    ...
    run() {
      ...
      return client.login(
        ...
        (err, data) => {
          ...
          that.robot.logger.info(
            `Logged in ${that.user_id} on device ${that.device_id}`
          );
          ...
          that.client.on("sync", (state, prevState, data) => {
            switch (state) {
              case "PREPARED":
                that.robot.logger.info(
                  `Synced ${that.client.getRooms().length} rooms`
                );
                ...
                const currentDisplayName = that.client.getUser(
                  that.user_id
                ).displayName;
                if (that.robot.name !== currentDisplayName) {
                  that.robot.logger.info(
                    `Setting display name to ${that.robot.name}`
                  );
            ...
            if (
              ...
              if (name !== that.user_id) {
                ...
                if (
                  ...
                  message.body.indexOf(that.robot.name) !== -1
                ) {
                  return that.client.sendReadReceipt(event);
                }
              }
            }
          });
          that.client.on("RoomMember.membership", async (event, member) => {
            if (
              member.membership === "invite" &&
              member.userId === that.user_id
            ) {
              await that.client.joinRoom(member.roomId);
              that.robot.logger.info(`Auto-joined ${member.roomId}`);
            }
          });
